...
class Sphere:
    ...
    def intersect(self, O, D):
        ...
        disc = (b ** 2) - (4 * c)
        sq = float32(squareroot(float32(np.maximum(0, disc))))
        h0 = (-b - sq) / 2
        h1 = (-b + sq) / 2
...
t0 = time.time()
def progra(xcam,ycam,zcam,numb):
    Q = vec3(x, y, 0)
#    E = vec3(0., 0.35, -1.)
    E = vec3(xcam, ycam, zcam)
    ...
    Image.merge("RGB", rgb).save("mpcuda{}.png".format(numb))
    
if __name__ == '__main__':
    procs = []
    for number in range(1,20):
        proc = Process(target=progra,args=(0., 0.35, -1.+(number*.01),number))
        procs.append(proc)
        proc.start()
    for proc in procs:
        proc.join()
